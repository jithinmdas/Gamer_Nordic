...
#define MULTILINK_PERIPHERAL_BASE_UUID    {{0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80,\
                                           0x00, 0x10, 0x00, 0x00, 0x00, 0x18, 0x00, 0x00}}  /**< Peripheral base UUID. */
#define MULTILINK_PERIPHERAL_SERVICE_UUID 0x2220                                            /**< Peripheral service UUID. */
#define MULTILINK_PERIPHERAL_CHAR_UUID    0x2221                                            /**< Peripheral characterisctics UUID. */
    ...
    if ((p_client != NULL) && (p_client->state == STATE_RUNNING))
    {
		if ((p_ble_evt->evt.gattc_evt.params.hvx.handle == p_client->srv_db.services[0].charateristics[p_client->char_index].characteristic.handle_value)&&(p_ble_evt->evt.gattc_evt.params.hvx.len == 1))
        {
            if(index < LEDS_NUMBER)
            {
				...
				char tag = p_ble_evt->evt.gattc_evt.params.hvx.data[0];
//				int16_t ax = (p_ble_evt->evt.gattc_evt.params.hvx.data[1])|(p_ble_evt->evt.gattc_evt.params.hvx.data[2] << 8);
//				int16_t ay = (p_ble_evt->evt.gattc_evt.params.hvx.data[3])|(p_ble_evt->evt.gattc_evt.params.hvx.data[4] << 8);
//				int16_t az = (p_ble_evt->evt.gattc_evt.params.hvx.data[5])|(p_ble_evt->evt.gattc_evt.params.hvx.data[6] << 8);
